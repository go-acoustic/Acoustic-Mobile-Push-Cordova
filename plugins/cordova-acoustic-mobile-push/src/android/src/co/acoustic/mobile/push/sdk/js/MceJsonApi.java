 ...
 */
 ...
 public class MceJsonApi {
    ...
    private static final String TAG = "MceJsonApi";
     ...
     private static void checkDuplicateSetAttribute(Context context, List<Attribute> attributes) throws JSONException {
         ...
         if(duplicateAttributes.size() > 0) {
             ...
             response.put("domain", true ? "user" : "channel");
             response.put("attributes", attributesJson);
             response.put("error", "The operation couldn’t be completed. (Duplicate " + (true ? "user" : "channel") + " attribute value updated error 101.)");
     ...
     public static void queueUpdateUserAttributes(Context context, JSONArray parameters) throws JSONException {
         ...
         checkDuplicateSetAttribute(context, attributes);
